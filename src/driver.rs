...
//! 
//! Drivers are instantiated by application code and passed to the [`crate::UsbHost::poll`] function.
//! 
...
//! 2. When enumeration has succeeded, the host calls [`Driver::attached`], informing drivers that a new device is available, and enters **discovery** phase
...
//!    the configurations that the device supports. All of these descriptors are parsed into `descriptor_type` and `data` and passed to the [`Driver::descriptor`] method one-by-one.
...
//!    The discovery logic separates these descriptors and passes each of them to the [`Driver::descriptor`] method separately.
//! 4. When all descriptors have been fetched, the host enters the **configuration** phase.
//! 5. During configuration, the host calls [`Driver::configure`] on each of the drivers *until one of them returns a value*.
//!    The value must be a valid configuration value (i.e. come from a [`ConfigurationDescriptor::value`]).
//! 6. If all of the drivers' `configure` calls returned `None` (no driver is interested in it), the host enteres **dormant** state.
//!    Otherwise the host calls [`Driver::configured`] on *all* of the drivers and enteres **configured** state.
//! 7. The [`Driver::configured`] callback informs the driver about the chosen configuration, and gives access to the host interface,
...
//!     fn turn_on_led<B: HostBus>(&mut self, host: &mut UsbHost<B>) -> Result<(), WouldBlock> {
...
//! ```
//! 
//! During the **enumeration phase** drivers are not yet involved. Once the host has assigned an
//! address to a new device, the **discovery phase** is started.
//! 
//! During the **discovery phase** the driver will receive:
//! - [`Driver::attached`] as soon as an address is assigned
//! - [`Driver::descriptor`] for each of the descriptors that are received from a device
//! 
//! 
//!
